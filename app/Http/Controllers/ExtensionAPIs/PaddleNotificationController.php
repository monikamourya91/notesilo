...
class PaddleNotificationController extends Controller
{
    ...
    {   
        ...
        $admin_plans = DB::table('plans')->where('level',1)->where('live_plan_id','!=',0)->get();
        $adminSubscriberPlansArray = $adminSubscriberPlansMappingArray = array();
        ...
        if(in_array($data["subscription_plan_id"], $adminSubscriberPlansArray)){
            //subscriber plans
        ...
        if($data["alert_name"] == "subscription_created"){
            if(isset($data["subscription_id"])){
                ...
                if(!empty($existingUser)){
                    ...
                    if($existingPlanData->plan_id == 1){
                        $action = "upgraded";
                        $plan_data = plan::where('id',$plan_id)->get(['name'])->first();
                        $plan_name = $plan_data->name;
                        sendUpgradeDowngradeMail($existingUser->email,$existingUser->name,$plan_name, $action);
                    }
                    ...
                    $userPassword = strtolower(generateRandomString(5));
                    ...
                    if($newUser_id > 0){
                        ...
                        sendWelcomeMailNewPaddleUser($newUser_details->email,$userPassword, '');
        ...
        if($data["alert_name"] == "subscription_updated"){
            if(isset($data["subscription_id"])){
                ...
                $action = "";
                $existing_plan_data = DB::table('plans')->where('id',$existing_subscription->plan_id)->get(['leads_limit'])->first();
                $existing_plan_limit = $existing_plan_data->leads_limit;

                if($existing_plan_limit < $plan_data->leads_limit){
                    $action = "upgraded";
                }elseif($existing_plan_limit > $plan_data->leads_limit){
                    $action = "downgraded";
                }
