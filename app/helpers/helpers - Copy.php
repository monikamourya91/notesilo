...
/*function addSubscriber($subscribers, $autoresponder_credentials) {
  $response = true;
  //return $autoresponder_credentials;
  switch($autoresponder_credentials['type']){
      case "mailerlite":
      if(verifyMailerLiteCredentials($autoresponder_credentials['field_two_value'],$autoresponder_credentials['field_one_value'])){     
    ...
    case "mailchimp":
      if(verifyMailChimpCredentials($autoresponder_credentials['field_two_value'],$autoresponder_credentials['field_one_value'])){      
        foreach($subscribers as $subscriber){
          $data = [
            'email' => strtolower($subscriber["email"]),
            'firstname' => $subscriber["first_name"],
    ...
    case "getresponse":
      $headers = array(
        'Content-Type: application/json',
        'X-Auth-Token: api-key '.$autoresponder_credentials['field_two_value']
      );

      $result = json_decode(getCompaignId($headers, $autoresponder_credentials['field_one_value']), true);
      if(isset($result["httpStatus"]) && ($result["httpStatus"] == 401)){
        $response = false;
      }elseif(sizeof($result) > 0){
      ...
      case "convertkit":
        ...
        foreach($subscribers as $subscriber){
          $data = [
            ...
            ,"fields" => array("last_name" => $subscriber["last_name"])
          ];
    ...
    case "klaviyo":
      if(verifyKlaviyoCredentials($autoresponder_credentials['field_two_value'], $autoresponder_credentials['field_one_value'])){    
        $data = array();
        foreach($subscribers as $subscriber){
          $data[] = [
              'email' => strtolower($subscriber["email"]),
    ...
    case "sendinblue":
      if(verifySendInBlueCredentials($autoresponder_credentials['field_two_value'],$autoresponder_credentials['field_one_value'])){   
        $data = array();
    ...
    case "acellemail":
      ...
      case "constantcontact":
          ...
          if(verifyConstantContactCredentials($autoresponder_credentials['field_two_value'],$autoresponder_credentials['field_one_value'],$autoresponder_credentials['field_three_value'])){   
            $data = array();
            foreach($subscribers as $subscriber){
              $data = [
                'email' => strtolower($subscriber["email"]),
    ...
    break;
     ...
     case "drip":
          ...
          if(verifyDripCredentials($autoresponder_credentials['field_one_value'],$autoresponder_credentials['field_two_value'])){
    ...
    case "kartra":
        ...
        if(verifyKartraCredentials($autoresponder_credentials['field_three_value'], $autoresponder_credentials['field_two_value'], $autoresponder_credentials['field_one_value'])){             
          foreach($subscribers as $subscriber){
            $data = [
              'email' => strtolower($subscriber["email"]),
              'first_name' => $subscriber["first_name"],
    ...
    case "sendgrid":
            if(verifySendGridCredentials($autoresponder_credentials['field_two_value'],$autoresponder_credentials['field_one_value'])){             
                $data = array();
            foreach($subscribers as $subscriber){
              $data[] = [
                'first_name' => $subscriber["first_name"],
    ...
    case "sendfox":
            if(verifySendFoxCredentials($autoresponder_credentials['field_two_value'],$autoresponder_credentials['field_one_value'])){             
                 foreach($subscribers as $subscriber){
                    $data = [
                      'email' => strtolower($subscriber["email"]),
                      'first_name' => $subscriber["first_name"],
    ...
    case "hubspot":
      if(verifyHubSpotCredentials($autoresponder_credentials['field_two_value'],$autoresponder_credentials['field_one_value'])){      
        foreach($subscribers as $subscriber){
          $data = [
            'email' => strtolower($subscriber["email"]),
            'firstname' => $subscriber["first_name"],
...
function getConvertKitTagId($autoresponder) {
  ...
  $ch = curl_init($url);
    ...
    $result = json_decode($result);
    foreach($result->tags as $tag){
      if($tag->name == $autoresponder['field_one_value']){
        return $tag->id;
      }
    }
    return false;
...
function addActiveCampaignContact($data,$autoresponder){
  ...
  $url = $autoresponder['field_three_value'].'/api/3/contacts/sync';
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json','Api-Token: '.$autoresponder['field_two_value']]);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  curl_setopt($ch, CURLOPT_TIMEOUT, 10);
...
function getAcContactId($data,$autoresponder){
  ...
  $url = $autoresponder['field_three_value'].'/api/3/contacts?email='.$data->contact->email;
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json','Api-Token: '.$autoresponder['field_two_value']]);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  curl_setopt($ch, CURLOPT_TIMEOUT, 10);
...
function addTagToContact($contactId,$autoresponder){
  ...
  if (is_array($activeCampaignTagIds)) {
    ...
    foreach ($activeCampaignTagIds as $oneTagObject) {
      ...
      $url = $autoresponder['field_three_value'].'/api/3/contactTags';
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json','Api-Token: '.$autoresponder['field_two_value']]);
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      curl_setopt($ch, CURLOPT_TIMEOUT, 10);
...
function syncActiveCampaign($contactId,$autoresponder){
  $url = $autoresponder['field_three_value'].'/api/3/contactLists';
  $data = json_encode([
    'contactList'=>[
        'list' => $autoresponder['field_one_value'],
        'contact' => $contactId,
        'status' => 1
      ]
    ]
...
function  createCustomFeildAndRelation($customFieldName,$autoresponder_credentials){
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL =>  $autoresponder_credentials['field_three_value']."/api/3/fields/",
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => "",
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
         ...
         if($httpCode ==201){
            ...
            $postDataTwo = array("fieldRel"=> [ "field"=> $newCustomFieldId, "relid"=> $listId]);
               ...
               $curl = curl_init();
                ...
                curl_setopt_array($curl, array(
                  CURLOPT_URL =>  $autoresponder_credentials['field_three_value']."/api/3/fieldRels",
                  CURLOPT_RETURNTRANSFER => true,
                  CURLOPT_ENCODING => "",
                  CURLOPT_MAXREDIRS => 10,
                  CURLOPT_TIMEOUT => 0,
...
function getCustomFieldIdAll($allQuestionArray,$autoresponder){
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL =>  $autoresponder['field_three_value'].'/api/3/fields',
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => "",
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
...
function syncSendInBlue($data, $autoresponder_credentials) { 
  ...
  $apiKey = $autoresponder_credentials['field_two_value']; 
  $listId = (int) $autoresponder_credentials['field_one_value'];
...
function syncConstantContact($data, $autoresponder_credentials) {
    $apiKey = $autoresponder_credentials['field_two_value']; 
    $listId = $autoresponder_credentials['field_one_value'];
    $token  = $autoresponder_credentials['field_three_value'];
...
function syncDripEmail($subscribers, $autoresponder_credentials) { 
  ...
  $account_id = $autoresponder_credentials['field_three_value']; 
...
function syncKartraEmail($subscriber, $autoresponder_credentials) { 
  ...
  $listName = $autoresponder_credentials['field_one_value'];
  $apiKey = $autoresponder_credentials['field_three_value'];
...
function syncSendGridEmail($subscriber, $autoresponder_credentials) { 
  $apiKey = $autoresponder_credentials['field_two_value'];
  $listId = array($autoresponder_credentials['field_one_value']);
...
function syncSendFoxEmail($subscriber, $autoresponder_credentials) { 
  $apiToken = $autoresponder_credentials['field_two_value'];
  $listId = array($autoresponder_credentials['field_one_value']);
...
function setValueToField($postData, $autoresponder_credentials){
      ...
      $json = json_encode($postData);  
            ...
            curl_setopt_array($curl, array(
              CURLOPT_URL =>  $autoresponder_credentials['field_three_value']."/api/3/fieldValues",
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => "",
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 0,
